 ...
 */
package scienceview.projection.temporal;
...
import scienceview.datamining.datarepresentation.RepresentationFactory;
import scienceview.datamining.datarepresentation.RepresentationType;
import scienceview.datamining.dimensionreduction.DimensionalityReduction;
import scienceview.datamining.dimensionreduction.DimensionalityReductionFactory;
import scienceview.datamining.dimensionreduction.DimensionalityReductionType;
import scienceview.datamining.networkanalysis.BibliographicCouplingConnectivity;
import scienceview.datamining.networkanalysis.CoAuthorshipConnectivy;
import scienceview.datamining.networkanalysis.CoreCitationConnectivity;
import scienceview.datamining.projection.ProjectionData;
import scienceview.datamining.projection.ProjectionFactory;
import scienceview.datamining.projection.ProjectionType;
import scienceview.datamining.termweight.MatrixTransformationFactory;
import scienceview.datarepresentation.graph.*;
import scienceview.projection.lsp.LSPProjection2D;
import scienceview.projection.lsp.MeshGenerator;
...
import scienceview.utils.distance.DissimilarityFactory;
import scienceview.utils.distance.DistanceMatrix;
import scienceview.utils.matrix.SparseMatrix;
import scienceview.utils.matrix.normalization.NormalizationFactory;
import scienceview.utils.stress.LoetStress;
...
public class TemporalGraphBuilder {
		...
		Thread t = new Thread() {
			...
			public void run() {
				try {
					...
					// SparseMatrix complete_matrix =
					// RepresentationFactory.getInstance(pdata.getRepresentationType(),
					// corpus).getMatrix(pdata);
					// SparseMatrix complete_matrix =
					// tproj.getProjectionData().getMatrix();
					...
					pdata.setStressSeries(stress.calculate(complete_matrix,
							DissimilarityFactory.getInstance(pdata.getDissimilarityType()), graphs));
	...
	private TIntArrayList selectMeaningfulControlPoints(Pair[][] mesh, TIntArrayList removed, TIntArrayList fixed,
			TIntArrayList update, SparseMatrix omatrix) {
		...
		for (TIntIterator it = update.iterator(); it.hasNext();) {
			for (Pair neighbor : mesh[omatrix.getIndexWithId(it.next())]) {
				if (!used.contains(omatrix.getIds()[neighbor.index])
						&& fixed.contains(omatrix.getIds()[neighbor.index])) {
		...
		System.err.println("Proportion of meaningful control points: " + used.size() + "/" + omatrix.getRowsCount()
				+ "(" + (used.size() * 100) / omatrix.getRowsCount() + ")");

		return used;
	}

	private TIntArrayList toUpdate(Pair[][] mesh, TIntArrayList ids_remove, SparseMatrix omatrix,
		...
		// para serem atualizados os documentos devem ter mais de um documento
		// em sua vizinhança que foi removido
		...
		try {
			...
			if (pdata.getDimensionReductionType() != DimensionalityReductionType.NONE) {
				if (previous_full_matrix.getDimensions() > pdata.getTargetDimension()) {
					...
					DimensionalityReduction dr = DimensionalityReductionFactory
							.getInstance(pdata.getDimensionReductionType(), pdata.getTargetDimension());
					previous_full_matrix = dr.reduce(previous_full_matrix, this.tproj);
				}
			} else if (pdata.getRepresentationType() != RepresentationType.LDA) {
				previous_full_matrix = MatrixTransformationFactory.getInstance(pdata.getMatrixTransformationType())
						.tranform(previous_full_matrix, null);
			...
			if (!pdata.getDistanceMatrixFilename().isEmpty()) {
				...
				distanceMatrix.save(
						pdata.getDistanceMatrixFilename().substring(0, pdata.getDistanceMatrixFilename().indexOf("."))
								.concat("-" + years[years.length - 1]).concat(".dmat"));
			}
			...
			for (int i = years.length - 2; i > 0; i--) {
				...
				update = new TreeMap<>(); // documentos que devem ser
											// atualizados
				ids_fixed = new TIntArrayList(); // documentos que devem
													// permanecer fixos
				ids_remove = Utils.toArrayList(corpus.getDocumentsIds(years[i + 1])); // documentos
																						// que
																						// devem
																						// ser
																						// removidos
				...
				if (!pdata.getDistanceMatrixFilename().isEmpty()) {
					DistanceMatrix distanceMatrix = new DistanceMatrix(current_full_matrix,
							DissimilarityFactory.getInstance(pdata.getDissimilarityType()));
					distanceMatrix.save(pdata.getDistanceMatrixFilename()
							.substring(0, pdata.getDistanceMatrixFilename().indexOf(".")).concat("-" + years[i])
							.concat(".dmat"));
				}
				...
				if (current_full_matrix.getRowsCount() <= 3) {
					...
					// se não existem pontos de controles suficientes, escolhar
					// mais entre os a serem alterados
					if (ids_fixed.size() < min_cp) {
						this.chooseAdditionalControlPoints(update, ids_update, ids_fixed, previous_full_matrix,
								projection);
					}

					System.err.println("Year: " + years[i]);
					meaningful_cp = this.selectMeaningfulControlPoints(mesh, ids_remove, ids_fixed, ids_update,
							previous_full_matrix);
					current_reduced_matrix = this.getMatrix(current_full_matrix, years[0], years[i], ids_update,
							meaningful_cp);

					// pegando a posição dos documentos que devem ficar fixos
					...
					if (!this.reduced_number_of_control_points || ids_fixed.size() == meaningful_cp.size()
							|| meaningful_cp.size() <= 10) {
						...
						if (ids_fixed.size() > meaningful_cp.size()) {
							...
							projection = this.mergeMatrices(projection1, current_reduced_matrix.getIds(),
									fixed_points_projection, aux);
						}
						createGraph(tproj, projection, current_full_matrix, corpus, years[i]);
					}
	...
	private void createGraph(TemporalProjection tproj, double[][] projection, SparseMatrix matrix,
		...
		tproj.addConnectivity(year,
				(new BibliographicCouplingConnectivity(corpus)).getBibliographicCoupling(graph.getVertex()));
	...
	// FileWriter(con.getName().concat("_").concat(Integer.toString(year)).concat(".txt"))))
	// {
	...
	// Vertex source = edge.getSource();
	// Vertex target = edge.getTarget();
	// writer.write(Integer.toString(index) + ": " + (source.getX() - min) /
	// (max - min) + " " + (source.getY() - min) / (max - min) + " " +
	// (target.getX() - min) / (max - min) + " " + (target.getY() - min) / (max
	// - min));
	// writer.newLine();
	// writer.flush();
	// index++;
	...
	//
	// } catch (IOException ex) {
	// Logger.getLogger(TemporalGraphBuilder2LSP.class.getName()).log(Level.SEVERE,
	// null, ex);
	// }
	// }
	private void createIntermediateGraphs() {
		...
		for (int i = 0; i < tproj.getNumberOfYears(); i++) {
			...
			if (entry != null) {
				...
				this.createIntermediateGraphs(year, graph1, graph2, this.newDocuments.get(year),
						this.updatedDocuments.get(year), this.fixedDocuments.get(year), this.usedCPDocuments.get(year));
			} else {
				TemporalGraph graph;
				...
				for (int j = 0; j < TemporalProjection.getN(); j++) {
					...
					// if (i == years.length-1 && j == TemporalProjection.getN()
					// - 1) {
					...
					while (iterator.hasNext()) {
						...
						if (j != 0) {
							...
							newVertex.setScalar(syear,
									Utils.indexOf(tproj.getYears(), newVertex.getPublishedYear()) * increment);
	...
	private void createIntermediateGraphs(Integer year, TemporalGraph graph1, TemporalGraph graph2,
			TIntArrayList newDocuments, TIntArrayList updatedDocuments, TIntArrayList fixedDocuments,
			TIntArrayList usedControlPoints) {
		ArrayList<TemporalGraph> graphsAux = new ArrayList<>();
		TemporalGraph graph;
		TIntObjectHashMap<Vertex> vertex;
	...
	private SparseMatrix getMatrix(SparseMatrix omatrix, int begin_year, int end_year, TIntArrayList update,
			TIntArrayList control_points) throws IOException {
	...
	private void chooseAdditionalControlPointsOutsideRegion(Geometry region, TreeMap<Integer, TIntArrayList> update,
			TIntArrayList updateSelected, TIntArrayList fixed, SparseMatrix omatrix, double[][] projection,
			double[][] projection_cp) {
			...
			for (TIntIterator it1 = updateSelected.iterator(); it1.hasNext();) {
				...
				if (!region.contains(geometryFactory.createPoint(new Coordinate(x_vertex, y_vertex)))) {
					...
					for (TIntIterator it = neighboors.iterator(); it.hasNext();) {
						...
						sum += Math.sqrt(Math.pow(x_vertex - projection[neighboor_index][0], 2)
								+ Math.pow(y_vertex - projection[neighboor_index][1], 2));
					}
	...
	private void chooseAdditionalControlPoints(TreeMap<Integer, TIntArrayList> update, TIntArrayList updateSelected,
			TIntArrayList fixed, SparseMatrix omatrix, double[][] projection) {
		ArrayList<ControlPointCandidate> candidates;
	...
	//
	// private void saveTrajectory(TreeMap<Integer, TemporalGraph> graphs,
	// ProjectionData pdata) {
	// BufferedWriter out = null;
	// StringBuilder aux = null;
	// try {
	// out = new BufferedWriter(new
	// FileWriter(pdata.getCollectionName().concat("_traj.txt")));
	// //writing the collection name
	// out.write(pdata.getCollectionName());
	// out.newLine();
	// //writing the years
	// out.write(graphs.keySet().toString().replace("[", "").replace("]",
	// "").replace(",", ";"));
	// out.newLine();
	//
	// int id;
	...
	// out.close();
	// } catch (IOException ex) {
	// Logger.getLogger(TemporalGraphBuilder.class.getName()).log(Level.SEVERE,
	// null, ex);
	// }
	// }
	private void saveData(ProjectionData pdata, SparseMatrix matrix, Corpus corpus) throws Exception {
		...
		if (corpus != null && pdata.getPExFilename().trim().length() > 0) {
			corpus.saveToPExFormat(pdata.getPExFilename(), pdata.isIndividualFilesToPExFormat(),
					pdata.getYearStepToPExFormat(), pdata);
		}
	}
